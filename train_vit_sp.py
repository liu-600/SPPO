...
def main(args):
    ...
    num_classes = len(train_dataset.dataset.classes)#64
    ...
    if 'channel' in args.prompt_mode:
        print("text_dim:",text_dim,"feature_dim:",feature_dim)# 512 384
    ...
    print("student.parameters():",student.parameters())
    ...
    if args.init:#在这载入
        checkpoint = torch.load(args.init, map_location=f'cuda:{args.gpu}')# 加载预训练参数
        student.load_state_dict(checkpoint['state_dict'], strict=False)# 放进模型
    else:
...
def train(text, student, train_loader, optim, epoch, args):
    ...
    for idx, episode in enumerate(train_loader):
        ...
        sup, que = sup.view(-1, *sup.shape[2:]), que.view(-1, *que.shape[2:])#[5, 3, 224, 224] [75, 3, 224, 224]
        ...
        if args.prompt_mode == 'spatial':
            ...
            _, sup_im_features = student.forward_with_semantic_prompt(sup, text_features, args)# support输入训练时加入语义引导
        else:
            _, sup_im_features = student.forward_with_semantic_prompt_channel(sup, text_features, args)#[5, 384]
        sup_im_features = sup_im_features.view(args.train_way, args.shot, -1).mean(dim=1)# 计算原型 [5, 384]
        _, que_im_features = student(que)# query时不用加语义引导 [75, 384]
    ...
    with torch.no_grad():
        for episode in test_loader:
            if args.aug_support == 1:
                ...
                if args.prompt_mode == 'spatial':
                    text_features = student.t2i(text_features)
                    _, sup_im_features = student.forward_with_semantic_prompt(sup, text_features, args)# S 加入语义引导来辅助提取特征
                else:
                    _, sup_im_features = student.forward_with_semantic_prompt_channel(sup, text_features, args)
                _, que_im_features = student(que)# Q 不用加语义引导
