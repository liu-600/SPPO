...
def main(args):
    ...
    args.tensorboard_dir = 'tensorboard/'+args.dataset+'/'+args.model+'/'+args.exp + '2/'
    args.checkpoint_dir = 'checkpoint/'+args.dataset+'/'+args.model+'/'+args.exp + '2/'
    ...
    if args.dataset == "CUB":
        ...
        train_dataset = CUB_FS(phase='train')
        dataset_val = CUB_FS(phase='val')
        data_loader = FewShotDataloader
        test_dataset = CUB_FS(phase='test')
        ...
        train_loader = data_loader(
            ...
            nTestNovel=args.way * 15,  # num test examples for all the novel categories
            nTestBase=args.way * 15,  # num test examples for all the base categories
            batch_size=25,
            num_workers=8,
            #epoch_size=args.episodes,  # num of batches per epoch
        )
        ...
        val_loader = data_loader(
            ...
            nKnovel=args.way,
            nKbase=0,
            nExemplars=args.way,  # num training examples per novel category
            nTestNovel=args.way * 15,  # num test examples for all the novel categories
    ...
    if args.optim == 'adam':
        ...
        optim = torch.optim.AdamW(student.parameters(), lr=args.lr, weight_decay=5e-2)#原5e-2
    else:
    ...
    for epoch in range(start_epoch, args.epochs):#800
        train(student, train_loader, optim, scheduler, epoch, args)

        if (epoch+1) % args.test_freq == 0 or (epoch+1)>40:#每5
        ...
        if args.cosine_annealing and (epoch+1) % args.annealing_period == 0:#   100
            lr = args.lr * args.gamma**int((epoch+1)/args.annealing_period)
            if args.optim == 'adam':
                optim = torch.optim.Adam(student.parameters(), lr=lr)
            elif args.optim == 'adamw':
                optim = torch.optim.AdamW(student.parameters(), lr=lr, weight_decay=5e-2)#原5e-2
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optim, T_max=args.annealing_period)
...
def train(student, train_loader, optim, scheduler, epoch, args):
    ...
    for idx, episode in enumerate(train_loader):
        ...
        # print("image:",image.size())#[128, 3, 84, 84]   ([128, 5, 3, 84, 84])
        # print("labels:",labels.size())#([128])  (128,5)
        image = image.view(-1,*image.shape[2:])
        labels = labels.view(-1)
        logit, features = student(image)# 特征提取+分类头64类
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_size', type=int, default=84, choices=[224, 84])
    parser.add_argument('--aug', action='store_true')
    parser.add_argument('--rand_aug', action='store_true')
    parser.add_argument('--repeat_aug', action='store_true')
    parser.add_argument('--model', type=str, default='resnet12', choices=['visformer-t', 'visformer-t-84','resnet12'])
