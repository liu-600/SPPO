...
def main(args):
    ...
    args.tensorboard_dir = 'tensorboard/' + args.dataset + '/' + args.model + '/' + args.exp + 'marginloss_spcam/'
    args.checkpoint_dir = 'checkpoint/' + args.dataset + '/' + args.model + '/' + args.exp + 'marginloss_spcam/'# 为了区分调了en_lr的权重，
    ...
    episode_sampler = EpisodeSampler(test_dataset.dataset.targets, args.episodes, args.way, args.shot + 50)#sne 15->50
    ...
    with torch.no_grad():
        ...
        for episode in test_loader:
            ...
            if args.aug_support == 1:# aug_support = 1
                ...
                image = episode[0].cuda(args.gpu)  # way * (shot+15)
                glabels = episode[1].cuda(args.gpu)
                labels = torch.arange(args.way).unsqueeze(-1).repeat(1, 50).view(-1).cuda(args.gpu)
                print("image:",image.size())
                image = image.view(args.way, args.shot + 50, *image.shape[1:])
                sup, que = image[:, :args.shot].contiguous(), image[:, args.shot:].contiguous()
                sup, que = sup.view(-1, *sup.shape[2:]), que.view(-1, *que.shape[2:])

                glabels = glabels.view(args.way, args.shot + 50)[:, :args.shot]
                glabels = glabels.contiguous().view(-1)
                text_features = text[glabels]
                ...
                sup_im_features = sup_im_features.view(args.train_way, args.shot, c, w, h).mean(dim=1)  # 计算原型 [5, 640,11,11]
                if args.sne =="sne":
                    emb_save(sup_im_features,que_im_features,i)
                fsup, fque = student.cam.forward(sup_im_features,que_im_features)  # 入[5, 640, 11, 11] [75, 640, 11, 11]-----出[1, 75, 5, 640, 11, 11] [1, 75, 5, 640, 11, 11]
                ...
                image = image.view(args.aug_support, args.way, args.shot + 50, *image.shape[1:])
                sup = image[:, :, :args.shot].contiguous().view(-1, *image.shape[3:])
                que = image[0, :, args.shot:].contiguous().view(-1, *image.shape[3:])

                glabels = glabels.view(args.way, args.shot + 50)[:, :args.shot]#[5,5]
                glabels_em = glabels
                glabels = glabels.unsqueeze(0).repeat(args.aug_support, 1, 1).contiguous().view(-1)
                text_features = text[glabels]
                # print("text_features:",text_features.size())#[750, 512]
                # print("sup:",sup.size())#[750, 3, 84, 84]
                ...
                _, que_im_features = student(que)
                # _, que_im_features = student.forward_with_semantic_prompt_channel(que, text_features, args)
                ####
                n, c, w, h = sup_im_features.size()
...
def emb_save(sup_im_features,que_im_features,i):#[5, 640, 11, 11] [75, 640, 11, 11]
    import numpy as np
    import pandas as pd
    ...
    que_im_features = que_im_features.mean(-1)  # #[75, 640,11,11]-> [75, 640]
    ...
    tsne_1 = tsne_norm[0:50]#很好
    tsne_2 = tsne_norm[502:550]
    tsne_3 = tsne_norm[550:600]
    tsne_4 = tsne_norm[600:650]
    tsne_5 = tsne_norm[650:700]
    # tsne_5 = tsne_5[0:37]
    # # print("tsne_norm[695,700]",tsne_norm[695:700])
    # tsne_5 = np.concatenate((tsne_5, tsne_norm[688:697]), axis=0)
    # tsne_5 = np.concatenate((tsne_5,tsne_norm[698:700]), axis=0)

    tsne_6 = tsne_norm[250:300]
    tsne_7 = tsne_norm[300:350]
    tsne_8 = tsne_norm[350:400]
    tsne_9 = tsne_norm[400:450]
    tsne_10 = tsne_norm[450:500]##很好
    ...
    # plt.scatter(tsne_3[:, 0], tsne_3[:, 1], 30,  label='Rusty Blackbird')
    # plt.scatter(tsne_4[:, 0], tsne_4[:, 1], 30,  label='Laysan Albatross')
    plt.scatter(tsne_5[:, 0], tsne_5[:, 1], 30,  label='Laysan Albatross')
    plt.scatter(tsne_6[:, 0], tsne_6[:, 1], 30,  label='Least Flycatcher')
    # tsne_normal[i, 0]为横坐标，X_norm[i, 1]为纵坐标，1为散点图的面积， color给每个类别设定颜色
    # plt.scatter(tsne_7[:, 0], tsne_7[:, 1], 30,  label='Warbling Vireo')
    # plt.scatter(tsne_8[:, 0], tsne_8[:, 1], 30,  label='Brewer Sparrow')
    plt.scatter(tsne_9[:, 0], tsne_9[:, 1], 30 , label='Brewer Sparrow')
    plt.scatter(tsne_10[:, 0], tsne_10[:, 1], 30, label='Warbling Vireo')
    ...
    plt.xticks( size=16)
    plt.yticks( size=16)
    plt.savefig("./Picture"+str(i)+".pdf",bbox_inches='tight')
    # plt.show()
