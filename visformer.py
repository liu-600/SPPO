...
class Visformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # head
        #print("x0:",x.size())#[128, 384, 7, 7]
        x = self.norm(x)
        #print("x1:", x.size())#[128, 384, 7, 7]
        if self.pool:
            x = self.global_pooling(x)
            #print("x2:", x.size())#[128, 384, 1, 1]
        else:
            x = x[:, :, 0, 0]
            #print("x3:", x.size())
        #print("XX:", x.size())#[128, 384, 1, 1]
        #print("head:", self.head)
        logit = self.head( x.view(x.size(0), -1) )#[128, 384】
        ...
        for b in self.stage2:
            if np.absolute(stage - args.stage) < 1e-6:
                ...
                if 'channel' in args.prompt_mode:#开始通道维度的融合
                    ...
                    print("context_stage2:",context.size())
        ...
        for b in self.stage3:
            if np.absolute(stage - args.stage) < 1e-6:
                ...
                if 'channel' in args.prompt_mode:
                    ...
                    context = torch.cat([context, prompt2], dim=-1)
                    print("context_stage3:", context.size())
                    context = self.se_block(context)
                    context = context - context.mean(dim=-1, keepdim=True)
