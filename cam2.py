...
class CAM2(nn.Module):
    ...
    def forward(self, f1, f2):# [5, 640, 11, 11] [75, 640, 11, 11]
        ...
        sup = f1.unsqueeze(dim=1).expand(-1, n2,-1,  -1, -1)
        sup = sup.reshape(b,n2,n1,c,w,h)
        que = f2.unsqueeze(dim=0).expand(n1, -1, -1, -1, -1)
        que = que.reshape(b, n2, n1, c, w, h)
        ...
        return sup, f2.transpose(1, 2) #[1, 75, 5, 640, 11, 11] [1, 75, 5, 640, 11, 11] 该样本针对每类的特征，每类对应该样本的特征
